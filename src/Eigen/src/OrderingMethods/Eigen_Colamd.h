...
*/
  template<typename Index>
  inline Index colamd_c(Index n_col) { return Index(((n_col) + 1) * sizeof(colamd_col<Index>) / sizeof(Index)); }

  template<typename Index>
  inline Index colamd_r(Index n_row) { return Index(((n_row) + 1) * sizeof(Colamd_Row<Index>) / sizeof(Index)); }

// Prototypes of non-user callable routines
  template<typename Index>
  static Index
  init_rows_cols(Index n_row, Index n_col, Colamd_Row<Index> Row[], colamd_col<Index> col[], Index A[], Index p[],
                 Index stats[COLAMD_STATS]);

  template<typename Index>
  static void
  init_scoring(Index n_row, Index n_col, Colamd_Row<Index> Row[], colamd_col<Index> Col[], Index A[], Index head[],
               double knobs[COLAMD_KNOBS], Index *p_n_row2, Index *p_n_col2, Index *p_max_deg);

  template<typename Index>
  static Index
  find_ordering(Index n_row, Index n_col, Index Alen, Colamd_Row<Index> Row[], colamd_col<Index> Col[], Index A[],
                Index head[], Index n_col2, Index max_deg, Index pfree);
  ...
  template<typename Index>
  static Index garbage_collection(Index n_row, Index n_col, Colamd_Row<Index> Row[], colamd_col<Index> Col[], Index A[],
                                  Index *pfree);
 ...
 */
  template<typename Index>
  static bool colamd(Index n_row, Index n_col, Index Alen, Index *A, Index *p, double knobs[COLAMD_KNOBS],
                     Index stats[COLAMD_STATS]) {
    /* === Local variables ================================================== */
    ...
    if (!stats) { COLAMD_DEBUG0 (("colamd: stats not present\n"));
      return (false);
    }
    ...
    if (n_row < 0)  /* n_row must be >= 0 */
    {
      ...
      stats[COLAMD_INFO1] = n_row;COLAMD_DEBUG0 (("colamd: nrow negative %d\n", n_row));
      return (false);
    }
    ...
    if (n_col < 0)  /* n_col must be >= 0 */
    {
      ...
      stats[COLAMD_INFO1] = n_col;COLAMD_DEBUG0 (("colamd: ncol negative %d\n", n_col));
      return (false);
    }
    ...
    if (nnz < 0)  /* nnz must be >= 0 */
    {
      ...
      stats[COLAMD_INFO1] = nnz;COLAMD_DEBUG0 (("colamd: number of entries negative %d\n", nnz));
      return (false);
    }

    if (p[0] != 0) {
      stats[COLAMD_STATUS] = COLAMD_ERROR_p0_nonzero;
      stats[COLAMD_INFO1] = p[0];COLAMD_DEBUG0 (("colamd: p[0] not zero %d\n", p[0]));
      return (false);
    }
    ...
    if (need > Alen) {
      ...
      stats[COLAMD_INFO2] = Alen;COLAMD_DEBUG0 (("colamd: Need Alen >= %d, given only Alen = %d\n", need, Alen));
      return (false);
    }
    ...
    stats[COLAMD_DEFRAG_COUNT] = ngarbage;COLAMD_DEBUG0 (("colamd: done.\n"));
    return (true);
  }
  ...
  static Index init_rows_cols  /* returns true if OK, or false otherwise */
    ...
    for (col = 0; col < n_col; col++) {
      ...
      if ((Col[col].length) < 0) // extra parentheses to work-around gcc bug 10200
      {
        ...
        stats[COLAMD_INFO2] = Col[col].length;COLAMD_DEBUG0 (("colamd: col %d length %d < 0\n", col, Col[col].length));
        return (false);
      }
    ...
    for (col = 0; col < n_col; col++) {
      ...
      while (cp < cp_end) {
        ...
        if (row < 0 || row >= n_row) {
          ...
          stats[COLAMD_INFO3] = n_row;COLAMD_DEBUG0 (("colamd: row %d col %d out of bounds\n", row, col));
          return (false);
        }
        ...
        if (row <= last_row || Row[row].shared2.mark == col) {
          ...
          stats[COLAMD_INFO1] = col;
          stats[COLAMD_INFO2] = row;
          (stats[COLAMD_INFO3])++;COLAMD_DEBUG1 (("colamd: row %d col %d unsorted/duplicate\n", row, col));
        }
    ...
    if (stats[COLAMD_STATUS] == COLAMD_OK_BUT_JUMBLED) { COLAMD_DEBUG0 (
...
*/
  ...
  static void init_scoring
    ...
    dense_col_count = std::max<Index>(0, (std::min)(Index(knobs[COLAMD_DENSE_COL] * n_row), n_row));COLAMD_DEBUG1 (
    ...
    for (c = n_col - 1; c >= 0; c--) {
      ...
      if (COL_IS_ALIVE (c)) { COLAMD_DEBUG4 (("place %d score %d minscore %d ncol %d\n",
...
*/
  ...
  static Index find_ordering /* return the number of garbage collections */
    ...
    min_score = 0;
    ngarbage = 0;COLAMD_DEBUG1 (("colamd: Ordering, n_col2=%d\n", n_col2));
    ...
    for (k = 0; k < n_col2; /* 'k' is incremented below */) {
      ...
      while (cp < cp_end) {
        /* get a row */
        row = *cp++;COLAMD_DEBUG4 (("Pivot col pattern %d %d\n", ROW_IS_ALIVE(row), row));
        /* skip if row is dead */
        if (ROW_IS_DEAD (row)) {
          continue;
        }
      ...
      while (cp < cp_end) {
        ...
        row = *cp++;COLAMD_DEBUG3 (("Kill row in pivot col: %d\n", row));
        KILL_ROW (row);
      }
      ...
      if (pivot_row_length > 0) {
        ...
        pivot_row = A[Col[pivot_col].start];COLAMD_DEBUG3 (("Pivotal row is %d\n", pivot_row));
      } else {
      ...
      while (rp < rp_end) {
        col = *rp++;
        COLAMD_ASSERT (COL_IS_ALIVE(col) && col != pivot_col);COLAMD_DEBUG3 (("Col: %d\n", col));
        ...
        while (cp < cp_end) {
          ...
          if (set_difference == 0) { COLAMD_DEBUG3 (("aggressive absorption. Row: %d\n", row));
            KILL_ROW (row);
          } else {
            /* save the new mark */
      ...
      while (rp < rp_end) {
        ...
        if (Col[col].length == 0) { COLAMD_DEBUG4 (("further mass elimination. Col: %d\n", col));
      ...
      tag_mark += (max_deg + 1);
      if (tag_mark >= max_mark) { COLAMD_DEBUG2 (("clearing tag_mark\n"));
        tag_mark = Eigen::internal::clear_mark(n_row, Row);
      }
