  ...
  */
  ...
  public:
    ...
    */
    ...
    operator*(const MatrixBase <OtherDerived> &a, const Transform &b) { return a.derived() * b.matrix(); }

    /** Contatenates two transformations */
    inline const Transform
    operator*(const Transform &other) const { return Transform(m_matrix * other.matrix()); }
    ...
    cast() const { return typename internal::cast_return_type<Transform, Transform<NewScalarType, Dim> >::type(*this); }

    /** Copy constructor with scalar type conversion */
    template<typename OtherScalarType>
    inline explicit
    Transform(const Transform<OtherScalarType, Dim> &other) { m_matrix = other.matrix().template cast<Scalar>(); }
...
*****************************************************/
  ...
  struct ei_transform_product_impl<Other, Dim, HDim, HDim, HDim> {
    ...
    static ResultType run(const TransformType &tr, const Other &other) { return tr.matrix() * other; }
  };
  ...
  struct ei_transform_product_impl<Other, Dim, HDim, Dim, 1> {
    ...
    static ResultType run(const TransformType &tr, const Other &other) {
      return ((tr.linear() * other) + tr.translation())
             * (Scalar(1) /
                ((tr.matrix().template block<1, Dim>(Dim, 0) * other).coeff(0) + tr.matrix().coeff(Dim, Dim)));
    }
  };
