...
// purely dynamic matrix.
  ...
  public:
    ...
    DenseStorage(DenseIndex size, DenseIndex nbRows, DenseIndex nbCols)
        : m_data(internal::conditional_aligned_new_auto<T, (_Options & DontAlign) == 0>(size)), m_rows(nbRows),
          m_cols(nbCols) { EIGEN_INTERNAL_DENSE_STORAGE_CTOR_PLUGIN }
...
// matrix with dynamic width and fixed height (so that matrix has dynamic size).
  ...
  public:
    ...
    DenseStorage(DenseIndex size, DenseIndex, DenseIndex nbCols) : m_data(
        internal::conditional_aligned_new_auto<T, (_Options & DontAlign) == 0>(size)), m_cols(
        nbCols) { EIGEN_INTERNAL_DENSE_STORAGE_CTOR_PLUGIN }
...
// matrix with dynamic height and fixed width (so that matrix has dynamic size).
  ...
  public:
    ...
    DenseStorage(DenseIndex size, DenseIndex nbRows, DenseIndex) : m_data(
        internal::conditional_aligned_new_auto<T, (_Options & DontAlign) == 0>(size)), m_rows(
        nbRows) { EIGEN_INTERNAL_DENSE_STORAGE_CTOR_PLUGIN }
