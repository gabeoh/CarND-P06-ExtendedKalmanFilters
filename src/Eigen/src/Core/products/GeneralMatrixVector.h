 ...
 */
    ...
    general_matrix_vector_product<Index, LhsScalar, ColMajor, ConjugateLhs, RhsScalar, ConjugateRhs, Version>::run(
        Index rows, Index cols,
        const LhsScalar *lhs, Index lhsStride,
        const RhsScalar *rhs, Index rhsIncr,
        ResScalar *res, Index resIncr, RhsScalar alpha) {
...
#define _EIGEN_ACCUMULATE_PACKETS(A0, A13, A2) \
    pstore(&res[j], \
      ...
      enum {
        AllAligned = 0, EvenAligned, FirstAligned, NoneAligned
      };
 ...
 */
    ...
    template<typename Index, typename LhsScalar, bool ConjugateLhs, typename RhsScalar, bool ConjugateRhs, int Version>
    EIGEN_DONT_INLINE void
    general_matrix_vector_product<Index, LhsScalar, RowMajor, ConjugateLhs, RhsScalar, ConjugateRhs, Version>::run(
        Index rows, Index cols,
        const LhsScalar *lhs, Index lhsStride,
        const RhsScalar *rhs, Index rhsIncr,
        ResScalar *res, Index resIncr,
...
#define _EIGEN_ACCUMULATE_PACKETS(A0, A13, A2) {\
    ...
    ptmp3 = pcj.pmadd(EIGEN_CAT(ploa,A13)<LhsPacket>(&lhs3[j]), b, ptmp3); }
      ...
      enum {
        AllAligned = 0, EvenAligned = 1, FirstAligned = 2, NoneAligned = 3
      };
      ...
      for (Index i = skipRows; i < rowBound; i += rowsAtOnce) {
        ...
        if (Vectorizable) {
          ...
          if (alignedSize > alignedStart) {
            switch (alignmentPattern) {
              case AllAligned:
                for (Index j = alignedStart; j < alignedSize; j += RhsPacketSize) _EIGEN_ACCUMULATE_PACKETS(d, d, d);
                break;
              case EvenAligned:
                for (Index j = alignedStart; j < alignedSize; j += RhsPacketSize) _EIGEN_ACCUMULATE_PACKETS(d, du, d);
                break;
              case FirstAligned: {
                Index j = alignedStart;
                if (peels > 1) {
                ...
                for (; j < alignedSize; j += RhsPacketSize) _EIGEN_ACCUMULATE_PACKETS(d, du, du);
                break;
              }
              default:
                for (Index j = alignedStart; j < alignedSize; j += RhsPacketSize) _EIGEN_ACCUMULATE_PACKETS(du, du, du);
                break;
            }
            tmp0 += predux(ptmp0);
            tmp1 += predux(ptmp1);
