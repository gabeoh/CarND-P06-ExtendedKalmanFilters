...
public:
  ...
  const Diagonal<const LazyCoeffBasedProductType, 0>
  diagonal() const { return reinterpret_cast<const LazyCoeffBasedProductType &>(*this); }

  template<int DiagonalIndex>
  const Diagonal<const LazyCoeffBasedProductType, DiagonalIndex>
  diagonal() const { return reinterpret_cast<const LazyCoeffBasedProductType &>(*this); }

  const Diagonal<const LazyCoeffBasedProductType, Dynamic>
  diagonal(Index index) const { return reinterpret_cast<const LazyCoeffBasedProductType &>(*this).diagonal(index); }
...
**************************************/
  ...
  struct product_coeff_impl<DefaultTraversal, 0, Lhs, Rhs, RetScalar> {
    ...
    run(Index /*row*/, Index /*col*/, const Lhs & /*lhs*/, const Rhs & /*rhs*/, RetScalar &res) {
      res = RetScalar(0);
    }
  };
...
*******************************************/
  ...
  struct product_coeff_vectorized_unroller {
    typedef typename Lhs::Index Index;
    enum {
      PacketSize = packet_traits<typename Lhs::Scalar>::size
    };

    static EIGEN_STRONG_INLINE void
    run(Index row, Index col, const Lhs &lhs, const Rhs &rhs, typename Lhs::PacketScalar &pres) {
  ...
  struct product_coeff_vectorized_dyn_selector {
    ...
    static EIGEN_STRONG_INLINE void
    run(Index row, Index col, const Lhs &lhs, const Rhs &rhs, typename Lhs::Scalar &res) {
      res = lhs.row(row).transpose().cwiseProduct(rhs.col(col)).sum();
    }
  };
...
*******************/
  ...
  struct product_packet_impl<RowMajor, 0, Lhs, Rhs, Packet, LoadMode> {
    ...
    static EIGEN_STRONG_INLINE void
    run(Index /*row*/, Index /*col*/, const Lhs & /*lhs*/, const Rhs & /*rhs*/, Packet &res) {
      res = pset1<Packet>(0);
    }
  };
