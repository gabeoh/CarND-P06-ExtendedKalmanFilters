...
// as usual if the result is row major => we transpose the product
    ...
    struct general_matrix_matrix_triangular_product<Index, LhsScalar, LhsStorageOrder, ConjugateLhs, RhsScalar, RhsStorageOrder, ConjugateRhs, ColMajor, UpLo, Version> {
      ...
      static EIGEN_STRONG_INLINE void run(Index size, Index depth, const LhsScalar *_lhs, Index lhsStride,
                                          const RhsScalar *_rhs, Index rhsStride, ResScalar *res, Index resStride,
                                          const ResScalar &alpha) {
        ...
        for (Index k2 = 0; k2 < depth; k2 += kc) {
          ...
          for (Index i2 = 0; i2 < size; i2 += mc) {
            ...
            if (UpLo == Upper) {
              ...
              gebp(res + resStride * j2 + i2, resStride, blockA, blockB + actual_kc * j2, actual_mc, actual_kc,
                   (std::max)(Index(0), size - j2), alpha,
                   -1, -1, 0, 0, allocatedBlockB);
            }
          }
        }
...
//   triangular traversal.
    ...
    struct tribb_kernel {
      ...
      void operator()(ResScalar *res, Index resStride, const LhsScalar *blockA, const RhsScalar *blockB, Index size,
                      Index depth, const ResScalar &alpha, RhsScalar *workspace) {
        ...
        for (Index j = 0; j < size; j += BlockSize) {
          ...
          if (UpLo == Lower) {
            ...
            gebp_kernel(res + j * resStride + i, resStride, blockA + depth * i, actual_b, size - i, depth,
...
// high level API
  ...
  TriangularView<MatrixType, UpLo>::assignProduct(const ProductBase <ProductDerived, _Lhs, _Rhs> &prod,
    ...
    general_product_to_triangular_selector<MatrixType, ProductDerived, UpLo,
        (_Lhs::ColsAtCompileTime == 1) || (_Rhs::RowsAtCompileTime == 1)>::run(m_matrix.const_cast_derived(),
                                                                               prod.derived(), alpha);
