...
protected:
  ...
  Eigen::Map<Matrix < Index, Dynamic, 1> >
...
public:
  ...
  void conservativeResize(Index rows, Index cols) {
    ...
    if (m_innerNonZeros) {
      ...
      Index *newInnerNonZeros = static_cast<Index *>(std::realloc(m_innerNonZeros,
                                                                  (m_outerSize + outerChange) * sizeof(Index)));
    ...
    Index *newOuterIndex = static_cast<Index *>(std::realloc(m_outerIndex,
                                                             (m_outerSize + outerChange + 1) * sizeof(Index)));
    if (!newOuterIndex) internal::throw_std_bad_alloc();
  ...
  inline SparseMatrix &operator=(const SparseSparseProduct <Lhs, Rhs> &product) { return Base::operator=(product); }
...
namespace internal {
  ...
  void set_from_triplets(const InputIterator &begin, const InputIterator &end, SparseMatrixType &mat, int Options = 0) {
    ...
    enum {
      IsRowMajor = SparseMatrixType::IsRowMajor
    };
...
template<typename Scalar, int _Options, typename _Index>
template<typename OtherDerived>
EIGEN_DONT_INLINE SparseMatrix<Scalar, _Options, _Index>
&

SparseMatrix<Scalar, _Options, _Index>::operator=(const SparseMatrixBase <OtherDerived> &other) {
  EIGEN_STATIC_ASSERT((internal::is_same<Scalar, typename OtherDerived::Scalar>::value),
                      YOU_MIXED_DIFFERENT_NUMERIC_TYPES__YOU_NEED_TO_USE_THE_CAST_METHOD_OF_MATRIXBASE_TO_CAST_NUMERIC_TYPES_EXPLICITLY)
...
&
SparseMatrix<_Scalar, _Options, _Index>::insertUncompressed(Index
row,
Index col
)
{
...
while ((p > startId) && (m_data.
index(p
-1) > inner))
{
m_data.
index(p) = m_data.index(p - 1);
m_data.
value(p) = m_data.value(p - 1);
--
p;
}
eigen_assert((p
<=startId || m_data.
index(p
-1)!=inner) && "you cannot insert an element that already exist, you must call coeffRef to this end");
...
EIGEN_DONT_INLINE typename SparseMatrix<_Scalar, _Options, _Index>::Scalar
&
SparseMatrix<_Scalar, _Options, _Index>::insertCompressed(Index
row,
Index col
)
{
...
{
m_outerIndex[previousOuter] = static_cast
<Index>(m_data
.
...
{
m_data.
index(k) = m_data.index(k - 1);
m_data.
value(k) = m_data.value(k - 1);
k--;
}
}
else
