...
namespace Eigen {
  ...
  inline BlockImpl(const XprType &xpr, int startRow, int startCol, int blockRows, int blockCols)
      : m_matrix(xpr), m_outerStart(IsRowMajor ? startRow : startCol),
  ...
  rows() const { return IsRowMajor ? m_outerSize.value() : m_matrix.rows(); }

  EIGEN_STRONG_INLINE Index

  cols() const { return IsRowMajor ? m_matrix.cols() : m_outerSize.value(); }
...
,BlockRows,BlockCols,true,Sparse>
...
inline const Scalar *valuePtr() const { return m_matrix.valuePtr() + m_matrix.outerIndexPtr()[m_outerStart]; }

inline Scalar *valuePtr() { return m_matrix.const_cast_derived().valuePtr() + m_matrix.outerIndexPtr()[m_outerStart]; }

inline const Index *innerIndexPtr() const { return m_matrix.innerIndexPtr() + m_matrix.outerIndexPtr()[m_outerStart]; }
...
inline Index *outerIndexPtr() { return m_matrix.const_cast_derived().outerIndexPtr() + m_outerStart; }
...
typename SparseMatrixBase<Derived>::InnerVectorReturnType SparseMatrixBase<Derived>::innerVector(Index
outer)
{
return

InnerVectorReturnType(derived(), outer
...
/** \returns the \a outer -th column (resp. row) of the matrix \c *this if \c *this
  ...
  */
template<typename Derived>
const typename SparseMatrixBase<Derived>::ConstInnerVectorReturnType SparseMatrixBase<Derived>::innerVector(Index
outer) const
{
return

ConstInnerVectorReturnType(derived(), outer
...
typename SparseMatrixBase<Derived>::InnerVectorsReturnType
    SparseMatrixBase<Derived>::innerVectors(Index
outerStart,
Index outerSize
)
{
...
Block<Derived, Dynamic, Dynamic, true>(derived(),
    IsRowMajor

? outerStart : 0, IsRowMajor ? 0 : outerStart,
IsRowMajor ? outerSize :
...
Block<const Derived, Dynamic, Dynamic, true>(derived(),
    IsRowMajor
...
public:
  ...
  EIGEN_STRONG_INLINE ReverseInnerIterator(const BlockType &block, Index outer)
    ...
    while ((Base::operator bool()) && (Base::index() >=
...
namespace internal {
  template<typename XprType, int BlockRows, int BlockCols, bool InnerPanel>
  class GenericSparseBlockInnerIteratorImpl<XprType, BlockRows, BlockCols, InnerPanel, false>
      : public internal::remove_all<typename XprType::Nested>::type::InnerIterator {
  public:
    typedef Block <XprType, BlockRows, BlockCols, InnerPanel> BlockType;
    enum {
  ...
  public:
    ...
    EIGEN_STRONG_INLINE GenericSparseBlockInnerIteratorImpl(const BlockType &block, Index outer)
        : Base(block.derived().nestedExpression(),
               outer + (IsRowMajor ? block.m_startRow.value() : block.m_startCol.value())),
          m_block(block),
          m_end(IsRowMajor ? block.m_startCol.value() + block.m_blockCols.value() : block.m_startRow.value() +
                                                                                    block.m_blockRows.value()) {
      while ((Base::operator bool()) &&
             (Base::index() < (IsRowMajor ? m_block.m_startCol.value() : m_block.m_startRow.value())))
        Base::operator++();
    }
  ...
  public:
    ...
    EIGEN_STRONG_INLINE GenericSparseBlockInnerIteratorImpl(const BlockType &block, Index outer = 0)
        :
        m_block(block),
        m_outerPos((IsRowMajor ? block.m_startCol.value() : block.m_startRow.value()) -
                   1), // -1 so that operator++ finds the first non-zero entry
    ...
    inline Index index() const {
      return m_outerPos - (IsRowMajor ? m_block.m_startCol.value() : m_block.m_startRow.value());
    }
